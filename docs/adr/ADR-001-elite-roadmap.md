...
### Phase 6 — learn-frames: visual ingest via Sonnet-vision keyframe captioning

- [x] **Written** — `crates/learn-frames/` crate: `FrameExtractor` (ffmpeg, 1 frame/10 s default, max-frames cap), `FrameCaptioner` (Anthropic vision, `claude-sonnet-4-6`, max_tokens=200, 429/503 backoff mirrors learn-synth), `extract_and_caption` high-level pipeline, `merge_segments` timestamp-sorted merge. `SegmentKind` enum (`Caption` | `FrameDescription`) added to `learn-core::Segment` with `#[serde(default)]` backward-compat. CLI: `--no-frames` opt-out, `--max-frames N` cap; frames enabled by default (`frames_enabled = !no_frames`). Acquisition path: `run_frame_captioning` wraps `extract_and_caption` as best-effort (skips gracefully if video file absent or API key missing). 2026-05-02.
- [x] **Ruflo-QA'd** — makes sense (visual → text → unified text pipeline; Sonnet descriptions richer than 512D embeddings); is working (7 new tests pass covering body construction, merge ordering, response parsing, fps-cap math, kind preservation, and backward-compat deserialization); makes a difference (closes the watch-skill regression — `learn ingest` now watches the video by default); tests legitimate (no vacuous tests; assertions on real values).
- [x] **Tested** — 234 tests passing (baseline 227, +7); all four workspace gates EXIT:0: `cargo fmt`, `cargo clippy -D warnings`, `cargo build --workspace`, `cargo test --workspace`. 2026-05-02.
- [x] **Confirmed** — `learn ingest <url> --topic <slug>` now calls `run_frame_captioning` after caption parsing; frame segments are merged by timestamp before chunking; `--no-frames` opt-out and `--max-frames N` cap wired to CLI.
